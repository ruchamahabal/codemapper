<?php
    ...
    function update_question($conn)
    {
      ...
      $sql = "update questions set question = '".$question."', initial_code = '".$initialCode."', function_name = '".$functionName."' where qid = ".$qid.";";
      ...
      {
          $sql = "delete from sample_test_cases where qid = ".$qid.";";
          mysqli_query($conn, $sql);
          $sql = "delete from testcase where qid = ".$qid.";";
          mysqli_query($conn, $sql);
          ...
          if(isset($_POST['sampleTestcases'])){
            for($i = 0; $i < sizeof($_POST['sampleTestcases']); $i++ )
            {
                ...
                $sql = "insert into testcase(tc_no, input, input_function_call, expected_output, qid, difficulty_level, type) values(".($i+1).",'".$testcase[0]."','".$input_func_call."','".$testcase[1]."',".$qid.",'".$testcase[2]."','sample');";
                if (mysqli_query($conn, $sql)) {
                  array_push($success_array, 1);
                } else {
                  array_push($success_array, 0);
          ...
          if(isset($_POST['hiddenTestcases'])){
            ...
            for($j = 0; $j < sizeof($_POST['hiddenTestcases']); $j++ )
            {
                ...
                $input_func_call = "print(".$_POST['functionName']."(".$testcase[0]."))";
                $sql = "insert into testcase(tc_no, input, input_function_call, expected_output, qid, difficulty_level, type) values(".$count.",'".$testcase[0]."','".$input_func_call."','".$testcase[1]."',".$qid.",".$testcase[2].",'hidden');";
                if (mysqli_query($conn, $sql)) {
                  array_push($success_array, 1);
                } else {
                  array_push($success_array, 0);
